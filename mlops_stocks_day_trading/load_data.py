...
class DataRepository:
    tickers_df: pd.DataFrame = None #main in-memory storage for tickers stats
    ...
    # PROTECTED: CREATE A TABLE IF NOT EXISTS 
    ...
    def _db_read_all_records_from_db(self):
        try:
            self.tickers_df = pd.read_sql('select * from tickers', self.db_conn)
            self.tickers_df['Date'] = pd.to_datetime(self.tickers_df['Date']) # need correct type 'datetime' and not object
        except Exception as e:
    ...
    def _fetch_ticker_incremental(self, ticker:str, period_days=1):
        ...
        incremental_df = retry(lambda: yf.download(ticker, start=start_date, end=end_date))
        incremental_df['Ticker']  = ticker #define a new field

        print(f'downloaded records {incremental_df.shape} with min_date {incremental_df.index.min()} and max_date {incremental_df.index.max()}')

        # https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.to_sql.html
        try:
            incremental_df.to_sql(name='tickers', 
                        index=True,
                        # index_label='',
                        if_exists='append',
                        con=self.db_conn)    
